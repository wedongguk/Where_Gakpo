...
def append_data(value, data):
    ...
    value.append(((data['Passes Fin 3rd'].sum() + data['KP'].sum() + data['PPA'].sum())/ (data['Min'].sum() / 90)) / (data['Passes Cmp'].sum() / (data['Min'].sum() / 90)) if data['Passes Cmp'].sum() != 0 else 0.0)
...
def fw_radar(player1, player2, player3, player4, player5):
    categories = ['Finishing', 'Assist', 'GCA', 'Defense', 'Ball Touch', 'Passing', 'Creative Pass', 'Ball Carry', 'Shooting']
    cody_value = []
    curtis_value = []
    dominik_value= []
    harvey_value= []
